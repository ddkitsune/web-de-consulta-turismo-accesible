        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 border-b border-gray-200">
                    <form method="POST" action="{{ route('tourism-routes.update', $tourism_route) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $tourism_route->title)" required autofocus />
                            ...
                            <div>
                                ...
                                <select id="location_id" name="location_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>
                                    ...
                                    @foreach($locations as $location)
                                        <option value="{{ $location->id }}" {{ old('location_id', $tourism_route->location_id) == $location->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>
                                    <option value="available" {{ old('status', $tourism_route->status) == 'available' ? 'selected' : '' }}>Disponible</option>
                                    <option value="unavailable" {{ old('status', $tourism_route->status) == 'unavailable' ? 'selected' : '' }}>No disponible</option>
                                </select>
                            ...
                            <!-- Imagen -->
                            <div>
                                <x-input-label for="image_file" value="Actualizar Imagen (Opcional)" />
                                @if($tourism_route->image)
                                    <div class="mb-2">
                                        <img src="{{ Storage::url($tourism_route->image) }}" class="w-32 h-32 object-cover rounded shadow">
                                    </div>
                                @endif
                            ...
                            <div>
                                ...
                                <x-text-input id="latitude" class="block mt-1 w-full" type="number" step="any" name="latitude" :value="old('latitude', $tourism_route->latitude)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="longitude" class="block mt-1 w-full" type="number" step="any" name="longitude" :value="old('longitude', $tourism_route->longitude)" />
                        ...
                        <div class="mt-6">
                            ...
                            <textarea id="description" name="description" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" rows="4" required>{{ old('description', $tourism_route->description) }}</textarea>
                        ...
                        <div class="mt-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                                @php
                                    $currentDisabilities = old('disabilities', $tourism_route->disabilities->pluck('id')->toArray());
                                @endphp
                                @foreach($disabilities as $disability)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="disabilities[]" value="{{ $disability->id }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array($disability->id, $currentDisabilities) ? 'checked' : '' }}>
